...
#For XGBoost
library(xgboost)
FCPC.train <- read.csv("RF_XG_raw_data_9groups.csv") # tibble::glimpse() 
FCPC.test <- read.csv("RF_XG_raw_data_9groups.csv") # please use the filename "RF_XG_raw_data_9groups.csv", which the test group of pig dataset is not summarized as Comp_ThB, to classify into 9 groups.
dim(FCPC.train) 
...
param <- list("objective" = "multi:softprob", 
              ...
              "num_class" = 9 # group/class (Fish_Con; Fish_Comp; Chicken_Con; Chicken_Comp; Pig_Con; Pig_Comp_ThB; Cattle_Before; Cattle_After)
)
...
pred <- matrix(pred,9,length(pred)/9)　# class (Fish_Con; Fish_Comp; Chicken_Con; Chicken_Comp; Pig_Con; Pig_Comp; Pig_ThB; Cattle_Con; Cattle_ThB)　　
pred <- t(pred)
colnames(pred)<-c("FE_Control","FE_Test","CW_Control","CW_Test","PK_Control","PK_Compost","PK_ThB","KC_Control","KC_Test") 
